...
use crate::{Commonality, DefaultCommonality, ZeroCommonality};
...
impl<K, Q, V, C> Index<&Q> for TotalHashMap<K, V, C>
where
    K: Eq + Hash + Borrow<Q>,
    Q: Eq + Hash + ?Sized,
{
...
impl<'a, K, V, C> IntoIterator for &'a mut TotalHashMap<K, V, C>
where
    C: Commonality<V>,
{
...
impl<K, V, C> Extend<(K, V)> for TotalHashMap<K, V, C>
where
    K: Eq + Hash,
    C: Commonality<V>,
{
...
impl<K, V, C> FromIterator<(K, V)> for TotalHashMap<K, V, C>
where
    K: Eq + Hash,
    C: Commonality<V>,
{
...
impl<K, V, C> PartialEq for TotalHashMap<K, V, C>
where
    K: Eq + Hash,
    V: PartialEq,
{
...
impl<K, V, C> Eq for TotalHashMap<K, V, C>
where
    K: Eq + Hash,
    V: PartialEq,
{
...
impl<K, V, C> Debug for TotalHashMap<K, V, C>
where
    K: Debug,
    V: Debug,
{
...
mod tests {
    ...
    #[test]
    fn iter_mut_drop() {
        let mut m = TotalHashMap::<i32, i32, DefaultCommonality>::new();
        m.iter_mut();
    }
}
